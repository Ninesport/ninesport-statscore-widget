...
describe("Widget Component with .env props", () => {
    ...
    // @ts-expect-error 不理他
    const lsportsEventId = import.meta.env.VITE_TEST_LSPORTS_EVENT_ID as string
    // @ts-expect-error 不理他
    const configurationId = import.meta.env.VITE_TEST_CONFIGURATION_ID as string
    // @ts-expect-error 不理他
    const widgetId = import.meta.env.VITE_TEST_WIDGET_ID as string
    // @ts-expect-error 不理他
    const language = import.meta.env.VITE_TEST_LANGUAGE as string
    ...
    beforeEach(() => {
        ...
        vi.mocked(scriptLoader.loadStatscoreScript).mockResolvedValue(undefined)
